...
本仓库是简体中文页面镜像，方便在浏览器中进行轻量协作；完整的来源归档、结构维护、日志、索引和简繁同步位于 [ndwiki-cn 主仓库](https://github.com/xyzhou0323/ndwiki-cn)。

### 可以直接在本仓库提交

- 错别字、措辞、格式和失效链接修正；
- 已有页面的局部补充或事实纠正；
- 来源已经在主仓库归档，并且不改变目录、标签和导航的单页修改或新增。

### 请改向主仓库提交

- 一次新增或重构多个相互关联的页面；
- 批量新增文献页、作者页或研究资料；
- 建立完整专题，或改变目录、页面边界、导航和标签体系；
- 来源尚未进入主仓库 `sources/`；
- 涉及医疗安全、自伤、自杀等需要重点来源复核的内容；
- 会引起大规模繁体同步的改动。

页面数量不是唯一标准。只要需要批量来源归档、结构设计或跨仓库同步，就属于 LLM Wiki 的“专题级摄入”，需要在主仓库中让原始资料、知识页面、schema、知识地图、`wiki-log.md` 和 `llm-wiki sync` 保持一致。

### 已经在本仓库提交了大型 PR

不需要重写或立即关闭 PR。请保留现有 PR，再基于已有内容向 `ndwiki-cn` 建立 PR，或先向维护者提供来源文件和来源清单。两个 PR 可以相互链接；主仓库版本完成后，当前 PR 再标记为由主仓库 PR 取代。
...
下面的步骤只适用于来源已经在主仓库归档、结构边界清晰的单页新增。完整专题、批量页面或需要新来源的页面，请向 [ndwiki-cn](https://github.com/xyzhou0323/ndwiki-cn) 提交。
...
`aliases`、`tags` 和 `sources` 使用块状列表。`sources` 必须指向 [ndwiki-cn](https://github.com/xyzhou0323/ndwiki-cn) 中实际归档的原始资料；如果需要新增来源，请从主仓库提交。
...
- 不确定放哪个文件夹？可以先提交草稿 PR 或 Issue，请维护者协助判断
- 一个页面聚焦一个主题；如果需要拆分为多个相互关联页面，请改走主仓库的专题级摄入流程
- 优先概括和总结，不要直接复制大段原文
...
A: 只要与神经多样性相关——概念解释、研究动态、人物介绍、文献摘要——都欢迎。不确定内容范围时可以先提交草稿 PR 或 Issue；不确定仓库时，少量页面修改可以先在这里讨论，多个页面或新来源请优先选择主仓库。

### Q: 为什么大型专题不能直接合并到这个仓库？

A: 本仓库只包含页面镜像。大型专题还需要归档原始来源、建立 `sources` 追溯、更新 schema 和知识地图、记录操作日志、运行索引同步并生成繁体版本；这些环节只能在完整的 `ndwiki-cn` 主仓库中统一完成。
